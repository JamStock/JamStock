...
const SignupScreen: React.FC<any> = ({ navigation }) => {
  ...
  const IdChecking = () => {
    //! 아이디 유효성 검사
    if (
      idText.length >= inputLength.nicknameLength.min &&
      idText.length <= inputLength.nicknameLength.max
    ) {
  ...
  const pwChecking = () => {
    //! 비밀번호 유효성 검사
    if (
      passwordText.length >= inputLength.pwcharLength.min &&
      passwordText.length <= inputLength.pwcharLength.max
    ) {
  ...
  const nickNameChecking = () => {
    //! 닉네임 유효성 검사
    if (
      nicknameText.length >= inputLength.nicknameLength.min &&
      nicknameText.length <= inputLength.nicknameLength.max
    ) {
  ...
  return (
    <View style={Styles.loginBox}>
      ...
      {/* 회원가입 완료 버튼 */}
      <TouchableOpacity
        style={Styles.signUpNFindBtn}
        onPress={() => {
          signUpCheck();
        }}
      >
        <Text
          style={{ fontSize: StylesText.sizeMedium.fontSize, textAlign: "center", marginTop: 9 }}
        >
          가입 완료
        </Text>
      </TouchableOpacity>
